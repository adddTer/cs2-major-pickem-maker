...
function getTeamId(t1: any): string | undefined {
    ...
    const matched = TEAMS.find(t => {
        ...
        const cleanStr = (s: string) => s.toLowerCase().replace(/^team\s+/i, '').replace(/\s+/g, '');
        ...
        return idLower === t1NameClean || 
               shortLower === t1AbbrClean || 
               idLower === t1AbbrClean || 
               tNameClean === t1NameClean ||
               t1NameClean.includes(idLower) || idLower.includes(t1NameClean) ||
               (t1AbbrClean && (shortLower.includes(t1AbbrClean) || t1AbbrClean.includes(shortLower)));
    });
...
function parseSwissGraph(stageData: any) {
    ...
    rounds.forEach(r => {
        ...
        ['groupAll', 'groupLow', 'groupMid', 'groupHigh'].forEach(gKey => {
            ...
            const bracket = group.name ? group.name.replace('-', ':') : GROUP_TO_BRACKET[r]?.[gKey];
            if (!bracket) return;
            ...
            group.matches.forEach((m: any) => {
                ...
                const format = m.format === '3' ? 'bo3' : (m.format === '5' ? 'bo5' : 'bo1');
                ...
                if (m.status === 'past' || m.status === 'live') {
                    if (format === 'bo1') {
                        ...
                        const maps = [];
                        if (m.t1?.bo1Score || m.t2?.bo1Score) maps.push({ score1: parseInt(m.t1?.bo1Score, 10) || 0, score2: parseInt(m.t2?.bo1Score, 10) || 0 });
                        if (m.t1?.bo2Score || m.t2?.bo2Score) maps.push({ score1: parseInt(m.t1?.bo2Score, 10) || 0, score2: parseInt(m.t2?.bo2Score, 10) || 0 });
                        if (m.t1?.bo3Score || m.t2?.bo3Score) maps.push({ score1: parseInt(m.t1?.bo3Score, 10) || 0, score2: parseInt(m.t2?.bo3Score, 10) || 0 });
                        if (m.t1?.bo4Score || m.t2?.bo4Score) maps.push({ score1: parseInt(m.t1?.bo4Score, 10) || 0, score2: parseInt(m.t2?.bo4Score, 10) || 0 });
                        if (m.t1?.bo5Score || m.t2?.bo5Score) maps.push({ score1: parseInt(m.t1?.bo5Score, 10) || 0, score2: parseInt(m.t2?.bo5Score, 10) || 0 });
                        if (maps.length > 0) matchObj.maps = maps;
                    }
                }
                
                if (newMatches[bracket] && !(t1Id === undefined && t2Id === undefined)) {
...
export async function fetchAndPatchCSGOData() {
    try {
        const fetchStage = async (id: string) => {
            const res = await fetch(`https://esports-data.5eplaycdn.com/v1/api/csgo/tournaments/${id}/stages`);
                ...
                try {
                    ...
                    if (eb && eb.matches && eb.rounds) {
                        ...
                        eb.rounds.forEach((r: any, idx: number) => {
                            ...
                            const matches = (r.matches || []).map((mStub: any) => eb.matches[mStub.absId || mStub.id]);
                            matches.forEach((m: any) => {
                                ...
                                const matchObj: any = { team1Id: t1Id, team2Id: t2Id, format: 'bo3' };
                                if (m.status === 'past' || m.status === 'live') {
                                    matchObj.score1 = parseInt(m.t1Score, 10) || 0;
                                    matchObj.score2 = parseInt(m.t2Score, 10) || 0;
                                    // Could add maps if they exist in playoffs data similar to swiss
                                }
