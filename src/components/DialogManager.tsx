...
export const dialog = {
    ...
    confirm: (message: string, title: string = "确认") => {
        return new Promise<boolean>((resolve) => {
            if (listener) {
                listener({ message, title, type: 'confirm', onConfirm: () => resolve(true), onCancel: () => resolve(false) });
            } else {
                resolve(confirm(message));
            }
        });
    },
    prompt: (message: string, defaultValue: string = "", title: string = "输入") => {
        return new Promise<string | null>((resolve) => {
             if (listener) {
                listener({ message, title, type: 'prompt', defaultValue, onConfirm: (val) => resolve(val || ''), onCancel: () => resolve(null) });
             } else {
                 resolve(prompt(message, defaultValue));
             }
        });
    },
    setListener: (l: DialogListener | null) => { listener = l; }
};
...
export const DialogManager: React.FC = () => {
    ...
    return (
        <Modal isOpen={true} onClose={current.type === 'alert' ? handleConfirm : handleCancel} title={current.title || '提示'}>
            <div className="flex flex-col gap-4">
                <div className="text-zinc-200 text-sm whitespace-pre-wrap">{current.message}</div>
