...
export const SlotBox: React.FC<{ 
  ...
  return (
     <div 
        ...
        onDrop={readOnly || isTbd ? undefined : (e) => onDrop && onDrop(e, slot.id)} 
        onDragOver={readOnly || isTbd ? undefined : e => { e.preventDefault(); e.dataTransfer.dropEffect = 'move'; }}
        onClick={readOnly || isTbd ? undefined : () => onClick && onClick(slot.id, slot.teamId)}
         className={cn(
           "rounded-[12px] bg-zinc-900/60 flex items-center justify-center transition-all duration-300 group relative border shrink-0", 
           !isExport && !isTbd && "backdrop-blur-md shadow-[inset_0_1px_4px_rgba(0,0,0,0.5)]",
           ...
           readOnly && !slot.resultStatus ? "" : (!readOnly && !isTbd) ? "hover:bg-zinc-800/80 cursor-pointer hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(0,0,0,0.5)]" : "",
           ...
           (team && !isTbd) ? cn("border-white/15 bg-zinc-800 hover:bg-zinc-700/80", !isExport && "shadow-md", border) : "border-white/20 hover:border-white/40 border-dashed bg-zinc-800/40 shadow-inner",
