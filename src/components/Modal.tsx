...
}> = ({ isOpen, onClose, title, children, maxWidthClass = "max-w-md md:max-w-2xl lg:max-w-[800px]", headerExtras, fullScreenOnMobile }) => {
    if (!isOpen) return null;
    return createPortal(
        <div className={cn("fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-sm", fullScreenOnMobile ? "p-0 md:p-4" : "p-4")}>
            <div className={cn(
                "bg-zinc-900 border-white/10 shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200", 
                fullScreenOnMobile ? "w-full h-full md:h-auto md:max-h-[90vh] md:rounded-xl md:border" : "rounded-xl border w-full max-h-[90vh]",
                maxWidthClass
            )}>
