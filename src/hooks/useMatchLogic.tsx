...
export function useMatchLogic(
    ...
    (
      ...
      return new Promise((resolve) => {
        ...
        Object.values(stageMatchesMap).forEach((batch) => {
          batch.forEach((m) => {
            if (
              m.team1Id &&
              m.team1Id !== "tbd" &&
              !allTeams.includes(m.team1Id)
            )
              allTeams.push(m.team1Id);
            if (
              m.team2Id &&
              m.team2Id !== "tbd" &&
              !allTeams.includes(m.team2Id)
            )
              allTeams.push(m.team2Id);
            ...
            if (
              ...
              if (m.score1 !== undefined && m.score2 !== undefined) {
                ...
                if (m.format === "bo1")
                  isComplete = m.score1 === 1 || m.score2 === 1;
                else if (m.format === "bo3")
                  isComplete = m.score1 === 2 || m.score2 === 2;
                else if (m.format === "bo5")
                  isComplete = m.score1 === 3 || m.score2 === 3;
        ...
        const worker = new Worker(
          new URL("../workers/simulateWorker.ts", import.meta.url),
          { type: "module" },
        );
        worker.onmessage = (e) => {
      ...
      Object.entries(matchesMap).forEach(([bracket, matches]) => {
        ...
        matches.forEach((m) => {
          ...
          if (m.team1Id) {
            ...
            if (newW + newL >= cur.w + cur.l)
              records[m.team1Id] = { w: newW, l: newL };
          }
          if (m.team2Id) {
            ...
            if (newW + newL >= cur.w + cur.l)
              records[m.team2Id] = { w: newW, l: newL };
          }
        });
      });
  ...
  const activeStageActuals = useMemo(
    () => getComputedActuals(activeStage),
    [activeStage, getComputedActuals],
  );
  ...
  const checkPrediction = useCallback(
    (
      ...
      if (stage === "playoffs") {
        ...
        const isInActuals = actuals.some(
          (a) => a.teamId === teamId && a.type === type,
        );
        const typeCount = actuals.filter((a) => a.type === type).length;
        const maxForType =
          type === "qf" ? 8 : type === "sf" ? 4 : type === "final" ? 2 : 1;
      ...
      if (type === "3-0" && actuals.filter((a) => a.type === "3-0").length >= 2)
        return "incorrect";
      if (type === "0-3" && actuals.filter((a) => a.type === "0-3").length >= 2)
        return "incorrect";
      if (
        type === "advance" &&
        actuals.filter((a) => a.type === "advance" || a.type === "3-0")
          .length >= 8
      )
        return "incorrect";
    ...
    (theirPicks: PickSlot[], stage: string, customFutures?: any[]) => {
      ...
      for (let i = 0; i < theirPicks.length; i++) {
        for (let j = i + 1; j < theirPicks.length; j++) {
          ...
          if (checkPrediction(p1.teamId, p1.type, stage) !== "unknown")
            continue;
          if (checkPrediction(p2.teamId, p2.type, stage) !== "unknown")
            continue;

          const isPlaying = scheduled.some(
            (m) =>
      ...
      const checkValidFutures = (f: any) =>
        f && (f.isPacked ? f.data.length > 0 : f.length > 0);
      const futuresToUse = checkValidFutures(customFutures)
        ? customFutures
        : simulatedFutures;
          ...
          futuresToUse.forEach((future) => {
            ...
            theirPicks.forEach((p) => {
              ...
              if (status === "correct") {
                ...
                else if (p.type === "0-3" && future.teams03.has(p.teamId))
                  score++;
                else if (
                  p.type === "advance" &&
                  future.teamsAdvance.has(p.teamId)
                )
                  score++;
              }
            });
      ...
      if (guaranteed >= 5) statusId = "passed";
        ...
        if (passingProbability !== undefined) {
          if (
            passingProbability >= 0.9 ||
            (passingProbability >= 0.7 && completedMatchesCount >= 8)
          )
            statusId = "great_chance";
          else if (
            (passingProbability <= 0.01 && completedMatchesCount >= 4) ||
            (passingProbability <= 0.1 && completedMatchesCount >= 10)
          )
            statusId = "slim_chance";
          else statusId = "uncertain";
        } else {
          const needed = 5 - Math.max(guaranteed, 0);
          const margin = possible - needed;
          if (margin >= 3 && completedMatchesCount >= 8)
            statusId = "great_chance";
          else if (margin === 0 && completedMatchesCount >= 4)
            statusId = "slim_chance";
          else statusId = "uncertain";
        }
      }
