    ...
    return (
        <div className="flex flex-col gap-2">
            <div 
                ...
                onClick={onToggleAll}
            >
                {allSelected ? <CheckSquare className="w-5 h-5 text-blue-400" /> : <Square className="w-5 h-5 text-zinc-500" />}
                <span className="font-bold text-sm text-zinc-200">全选 ({selectedIds.length}/{items.length})</span>
            </div>
...
export const HomeView: React.FC<{
  ...
  refreshPicks?: () => void;
}> = ({ newNickname, setNewNickname, communityPicks, handleCreateNew, handleEditExisting, refreshPicks }) => {
  ...
  return (
    <div className="flex-1 w-full flex flex-col bg-zinc-900/60 border border-white/5 rounded-lg shadow-xl relative backdrop-blur-md overflow-hidden max-w-2xl mx-auto p-4 sm:p-8">
        <div className="text-center space-y-2 mb-6 sm:mb-8 mt-2 sm:mt-4">
            <h1 className="text-xl sm:text-2xl font-black tracking-wide text-zinc-100">选择或创建预测 ID</h1>
            <p className="text-zinc-400 text-xs sm:text-sm">创建的竞猜将保存在本地。</p>
        </div>

        <div className="bg-zinc-950/50 p-4 sm:p-6 rounded-lg border border-white/5 shadow-inner mb-6 sm:mb-8 shrink-0">
            <h3 className="text-[10px] sm:text-xs font-bold text-zinc-500 uppercase tracking-widest mb-3 sm:mb-4">创建新预测</h3>
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
                <input 
                    ...
                    className="flex-1 bg-black/40 border border-white/10 rounded px-3 sm:px-4 py-2.5 sm:py-3 text-sm text-zinc-100 outline-none focus:border-blue-500/50 transition-colors"
                />
                <button 
                    onClick={handleCreateNew}
                    className="px-4 sm:px-6 py-2.5 sm:py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm transition-colors rounded-md flex items-center justify-center gap-2 shadow-lg shadow-blue-900/20"
                >
                    <Plus className="w-4 h-4" /> 创建竞猜
                </button>
            </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto flex flex-col custom-scrollbar">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 shrink-0">
                <h3 className="text-[10px] sm:text-xs font-bold text-zinc-500 uppercase tracking-widest">已有竞猜</h3>
        ...
        <Modal isOpen={showExportModal} onClose={() => setShowExportModal(false)} title="导出竞猜数据">
            <div className="flex flex-col min-h-0">
                <div className="overflow-y-auto pr-2 custom-scrollbar max-h-[50vh]">
                    <CheckboxList 
                        items={communityPicks.map(p => ({ id: p.id, name: p.name, date: new Date(p.createdAt).toLocaleString() }))}
                        selectedIds={exportSelectedIds}
                        onToggle={(id) => setExportSelectedIds(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id])}
                        onToggleAll={() => setExportSelectedIds(exportSelectedIds.length === communityPicks.length ? [] : communityPicks.map(p => p.id))}
                    />
                </div>
            </div>
        ...
        <Modal isOpen={showImportModal} onClose={() => setShowImportModal(false)} title="导入竞猜数据">
            <div className="flex flex-col min-h-0">
                <div className="overflow-y-auto pr-2 custom-scrollbar max-h-[50vh]">
                    <CheckboxList 
                        items={importedPicks.map(p => ({ id: p.id, name: p.name, date: new Date(p.createdAt).toLocaleString() }))}
                        selectedIds={importSelectedIds}
                        onToggle={(id) => setImportSelectedIds(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id])}
                        onToggleAll={() => setImportSelectedIds(importSelectedIds.length === importedPicks.length ? [] : importedPicks.map(p => p.id))}
                    />
                </div>
            </div>
            <div className="mt-4 pt-4 border-t border-white/5 flex justify-end shrink-0">
