...
import { StageKey, MatrixSet } from "../types";
...
import { Trash2, Copy, Plus, Save, Edit2, Menu, X, Wand2 } from "lucide-react";
...
export const SimulatorView: React.FC<SimulatorViewProps> = ({
  ...
  if (activeStage === "playoffs") {
    return <div className="p-8 text-center text-zinc-400">目前暂不支持决胜阶段的推演。</div>;
  }

  if (stageTeams.length < 16) {
    return <div className="p-8 text-center text-zinc-400">该阶段当前首轮对阵未完全确定，无法进行推演。</div>;
  }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden text-sm relative z-20">
      <div className="flex border-b border-white/5 items-center px-4 py-2 shrink-0 z-10 w-full bg-[#070b09]/80 backdrop-blur">
        {[
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className={cn(
          "absolute inset-y-0 left-0 z-50 w-64 md:w-56 border-r border-white/5 bg-zinc-950 flex flex-col shrink-0 transition-transform duration-200 ease-in-out md:relative md:translate-x-0",
          showMobileMenu ? "translate-x-0" : "-translate-x-full"
        )}>
          <div className="p-4 border-b border-white/5 shrink-0 flex items-center justify-between">
             <div className="flex items-center gap-2">
               <h2 className="font-bold text-zinc-300">概率矩阵</h2>
             </div>
             <div className="flex items-center gap-1">
               <button onClick={handleOpenQuickCreate} className="p-1 hover:bg-white/10 rounded text-sky-400" title="修改实力来快速创建矩阵">
                 <Wand2 className="w-4 h-4" />
               </button>
               <button onClick={handleCreateNew} className="p-1 hover:bg-white/10 rounded text-emerald-400" title="新建纯空白矩阵">
                 <Plus className="w-4 h-4" />
               </button>
               <button onClick={() => setShowMobileMenu(false)} className="md:hidden p-1 hover:bg-white/10 rounded text-zinc-400 ml-1">
                 <X className="w-4 h-4" />
               </button>
             </div>
          </div>
            ...
            {matrices.map(m => (
              <div 
                ...
                className={cn(
                  ...
                  selectedMatrixId === m.id ? "bg-zinc-800 text-white" : "hover:bg-zinc-900 text-zinc-400"
                )}
                onClick={() => {
              ...
              >
                ...
                <div className="flex items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                   <button onClick={(e) => { e.stopPropagation(); handleDuplicate(m); }} className="p-1.5 md:p-1 hover:text-emerald-400 bg-black/20 rounded md:bg-transparent" title="复制">
                      <Copy className="w-3.5 h-3.5" />
                   </button>
                   {!m.isDefault && (
                     <>
                     <button onClick={(e) => { e.stopPropagation(); handleRename(m); }} className="p-1.5 md:p-1 hover:text-blue-400 bg-black/20 rounded md:bg-transparent" title="重命名">
                        <Edit2 className="w-3.5 h-3.5" />
                     </button>
                     <button onClick={(e) => { e.stopPropagation(); handleDelete(m); }} className="p-1.5 md:p-1 hover:text-rose-400 bg-black/20 rounded md:bg-transparent" title="删除">
        ...
        {/* Main Content */}
        <div className="flex-1 flex flex-col bg-zinc-900/50 overflow-hidden relative">
          {activeMatrix ? (
             <div className="flex flex-col h-full p-2 md:p-4 relative z-20">
               <div className="flex flex-col md:flex-row md:items-center justify-between mb-2 md:mb-4 gap-2 md:gap-4 shrink-0">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 max-w-full">
                       <button onClick={() => setShowMobileMenu(true)} className="md:hidden p-1.5 -ml-1 hover:bg-white/10 rounded flex items-center justify-center shrink-0">
                         <Menu className="w-5 h-5 text-zinc-300" />
                       </button>
                       <h1 className="text-base md:text-lg font-bold text-white flex items-center gap-2 truncate">
                         {activeMatrix.name}
                         {activeMatrix.isDefault && <span className="px-1.5 py-0.5 bg-sky-500/20 text-sky-400 text-[9px] md:text-[10px] rounded border border-sky-500/30 shrink-0">系统只读</span>}
                       </h1>
                    </div>
                    <p className="text-[10px] md:text-xs text-zinc-500 mt-1 pl-7 md:pl-0">
                       此矩阵定义了BO1对决时的单图胜率。推演不含 "特定图elo+150" 机制，BO3会自动乘算。
                    </p>
                  </div>
                     <div className="flex items-center justify-end gap-2 shrink-0">
                       <button onClick={handleAIPickem} disabled={aiPickemState.isGenerating} className="px-3 md:px-4 py-1.5 md:py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-600/50 disabled:cursor-wait text-white rounded text-[10px] md:text-xs font-bold flex items-center gap-1.5 shadow-sm transition-colors shrink-0">
                          <Wand2 className="w-3.5 h-3.5 md:w-4 md:h-4" />
                          {aiPickemState.isGenerating ? "计算中..." : "AI最优作业"}
                       </button>
                       {!activeMatrix.isDefault && (
                         <button onClick={handleSaveLocal} className="px-3 md:px-4 py-1.5 md:py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded text-[10px] md:text-xs font-bold flex items-center gap-1.5 shadow-sm transition-colors shrink-0">
               ...
               <div className="flex-1 overflow-auto border border-white/10 rounded bg-[#070b09] custom-scrollbar">
                 <div className="inline-block min-w-max relative pb-4 w-full">
                   <div className="flex sticky top-0 z-20 bg-zinc-950 border-b border-white/5">
                      <div className="w-16 min-w-[64px] md:w-20 md:min-w-[80px] shrink-0 sticky left-0 z-30 bg-zinc-950 border-r border-white/5 flex items-center justify-center text-[10px] text-zinc-500 p-1 font-bold">
                        横(胜率) \ 纵
                      </div>
                      {stageTeams.map(t => (
                        <div key={`col-${t.id}`} className="flex-1 min-w-[48px] md:min-w-[52px] lg:min-w-[60px] shrink-0 flex items-center justify-center text-[10px] md:text-xs p-1 md:px-1.5 md:py-1 border-r border-white/5 bg-zinc-950" title={t.name}>
                          {t.logo ? (
                   ...
                   {stageTeams.map((rowTeam) => (
                     <div key={`row-${rowTeam.id}`} className="flex border-b border-white/5 hover:bg-white/5 transition-colors">
                       <div className="w-16 min-w-[64px] md:w-20 md:min-w-[80px] shrink-0 sticky left-0 z-10 bg-zinc-950 border-r border-white/5 flex flex-col md:flex-row items-center justify-center gap-0.5 md:gap-1.5 p-1 md:px-1.5">
                         {rowTeam.logo && (
                           <div className="w-4 h-4 md:w-5 md:h-5 shrink-0">
                             <TeamLogo team={rowTeam} fallbackClasses="text-[8px]" />
                           </div>
                         )}
                         <span className="text-[9px] md:text-xs md:font-medium truncate text-zinc-300 text-center md:text-left leading-none" title={rowTeam.name}>{rowTeam.shortName}</span>
                       </div>
                       ...
                       {stageTeams.map((colTeam) => {
                          ...
                          if (!isSelf) {
                            if (probPctNum > 65) valColor = "text-emerald-400 font-bold";
                            else if (probPctNum > 50) valColor = "text-emerald-300";
                            else if (probPctNum === 50) valColor = "text-zinc-400";
                            else if (probPctNum > 35) valColor = "text-rose-300";
                            else valColor = "text-rose-400 font-bold";
                          }

                          return (
                            <div key={`cell-${rowTeam.id}-${colTeam.id}`} className="flex-1 min-w-[48px] md:min-w-[52px] lg:min-w-[60px] shrink-0 border-r border-white/5 flex items-center justify-center relative group min-h-[40px] md:min-h-[40px]">
                              {isSelf ? (
                                <span className="text-zinc-700">-</span>
                              ) : (
                                activeMatrix.isDefault ? (
                                  <span className={cn("text-[10px] md:text-xs cursor-default selection:bg-transparent", valColor)}>
                                    {probPct}%
                                  </span>
                                ) : (
                                  <input 
                                    className={cn(
                                       "w-full h-full bg-transparent text-center text-[10px] md:text-xs outline-none focus:bg-white/10 transition-colors",
                                       valColor
                                    )}
                                    type="text"
                                    inputMode="numeric"
      ...
      >
        <div className="text-zinc-400 text-[11px] md:text-sm mb-4 leading-relaxed">
          通过调整以下队伍的实力值 (Elo) 来快速生成对应的概率矩阵。系统默认采用队伍此时的 VRS / HLTV 等效积分。如果该队伍没有找到相关积分，则会根据种子顺位进行分配。
        </div>

        <div className="mb-5 flex flex-col gap-2 p-3 bg-black/30 border border-white/5 rounded">
          <div className="text-sm font-medium text-zinc-300">
            基础实力计算比重 <span className="text-zinc-500 font-normal ml-2 text-xs">(调节将重置下方所有输入)</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-3 mb-6">
          {stageTeams.map(t => (
            <div key={t.id} className="flex items-center gap-3 p-2 bg-black/20 border border-white/5 rounded">
              <div className="w-5 h-5 shrink-0">
                <TeamLogo team={t} fallbackClasses="text-xs" />
              </div>
              <span className="flex-1 text-xs md:text-sm font-medium text-zinc-200 truncate">{t.name}</span>
              <input
                type="text"
                inputMode="numeric"
                pattern="[0-9]*"
                className="w-20 bg-zinc-950 border border-white/10 rounded px-2 py-1 text-xs md:text-sm text-right text-emerald-400 focus:outline-none focus:border-emerald-500/50"
        ...
        <div className="flex justify-end gap-2 shrink-0">
          <button
            ...
            className="px-4 py-2 rounded bg-white/5 hover:bg-white/10 text-zinc-300 text-sm font-medium transition-colors"
          >
            取消
          </button>
          <button
            onClick={handleQuickCreateSave}
            className="px-4 py-2 rounded bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-medium transition-colors shadow-sm"
          >
            生成矩阵
          </button>
        </div>
          ...
          {aiPickemState.isGenerating ? (
            <div className="flex flex-col items-center justify-center p-8 gap-4">
               ...
               <div className="text-sm font-bold text-zinc-300">
                 {aiPickemState.phase === 'simulating' ? '正在进行 10万 次矩阵推演...' : '正在启发式搜索最优化作业...'}
               </div>
               ...
               <div className="text-xs text-zinc-500">{aiPickemState.progress}%</div>
            </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-lg flex flex-col items-center justify-center gap-2">
                ...
                <span className="text-zinc-400 text-xs text-center">此概率基于当前选择的胜率矩阵进行10万次模拟并进行启发式搜索得出。</span>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-2">
                  <div className="text-xs font-bold text-zinc-500 uppercase tracking-widest pl-2">3-0 晋级</div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="text-xs font-bold text-zinc-500 uppercase tracking-widest pl-2">0-3 淘汰</div>
                  <div className="grid grid-cols-1 gap-2">
              ...
              <div className="flex flex-col gap-2">
                <div className="text-xs font-bold text-zinc-500 uppercase tracking-widest pl-2">其余晋级队伍</div>
